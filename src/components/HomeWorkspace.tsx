  ...
  return (
      <section id="workspace" aria-labelledby="workspace-heading" className="container mx-auto mt-10 max-w-3xl">
        ...
        >
          ...
          {image ? (
            <div className="mt-3 flex items-center gap-3 rounded-2xl border border-border/70 bg-background/80 p-3 backdrop-blur-sm">
          ...
          {(loading || result) && (
            <>
              ...
              {loading && streamPreview && isLivePreviewMode(streamMode ?? resultMode) ? (
                <p className="whitespace-pre-wrap rounded-[1.75rem] border border-border/70 bg-card/80 p-5 text-lg font-medium leading-relaxed backdrop-blur-sm">
                  {streamPreview}
                </p>
              ) : loading ? (
