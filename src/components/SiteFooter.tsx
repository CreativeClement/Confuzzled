...
export function SiteFooter() {
  return (
    <footer className="relative mt-8 overflow-hidden print:hidden">
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 hairline" />
        ...
        <nav aria-label="Footer" className="flex flex-wrap items-center justify-center gap-2">
          {LINKS.map((link) => (
            <Button
              ...
              key={link.href}
              variant="outline"
              className="rounded-full border-border/90 bg-background/80 px-5 shadow-sm"
            >
