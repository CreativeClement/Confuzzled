...
function Flashcard({
  ...
  return (
    ...
    >
      ...
      >
        <span className="absolute inset-0 flex flex-col justify-between rounded-2xl border border-border/80 bg-card/90 p-4 shadow-sm ring-offset-background backdrop-blur-sm group-hover:border-primary/40 group-focus-visible:ring-2 group-focus-visible:ring-ring [backface-visibility:hidden]">
        ...
        {data.map((item) => {
          ...
          return (
            <li key={`${item.step}-${item.text}`}>
              <article
                className={cn(
                  "rounded-2xl border border-border/80 bg-card/80 p-4 shadow-sm backdrop-blur-sm transition-all duration-200 sm:p-5",
                  checked && "border-primary/40 bg-primary/5 shadow-glow",
                )}
              >
